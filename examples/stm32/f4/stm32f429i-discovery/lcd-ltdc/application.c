...
static inline void draw_floodfill4(demo_mode_t demo_mode) {
	...
	gfx_set_clipping_area(x-20, y-20, x+FROWNY_WIDTH+20, y+FROWNY_HEIGHT+20);
	...
	if (demo_mode==DEMO_MODE_FLOODFILL4) {
		...
		if (scan_x>=0) gfx_draw_vline(x+scan_x, y, FROWNY_HEIGHT, GFX_COLOR_BLUE2);
		if (scan_y>=0) gfx_draw_hline(x, y+scan_y, FROWNY_WIDTH , GFX_COLOR_BLUE2);
	...
	fill_segment_queue_statistics_t stats = gfx_flood_fill4(x+40,y+50, ILI9341_LAYER2_COLOR_KEY, GFX_COLOR_RED, fill_segment_buf, sizeof(fill_segment_buf));
	...
	snprintf(buf,1023,
			...
			stats.overflows);
	buf[1023]=0;
	gfx_puts2(10, 198, buf, &font_Tamsyn5x9b_9 , GFX_COLOR_WHITE);
}
...
static inline void draw_point_list(point2d_t *points, size_t points_count, uint16_t _color) {
	color = _color;
...
static inline void draw_bezier(demo_mode_t demo_mode) {
	...
	switch (demo_mode) {
		...
		case DEMO_MODE_BEZIER :
			...
			mat3x3_affine_transform(mat, pentagram_, pentagram, num_points);
			...
			/* draw bezier */
			color = GFX_COLOR_RED;
			h2_bezier_draw_cubic(draw_segment, 20, curve_points[0], curve_points[1], curve_points[2], curve_points[3]);
			for (i=0; i<4; i++) {
				gfx_fill_circle(
				...
				buf[0]=48+i%10;
				gfx_puts2(
						(int16_t)(curve_points[i].x) - font_Tamsyn5x9b_9.charwidth *gfx_get_font_scale()/2+1,
						(int16_t)(curve_points[i].y) - font_Tamsyn5x9b_9.lineheight*gfx_get_font_scale()/2,
						buf,
						&font_Tamsyn5x9b_9,
						GFX_COLOR_WHITE
					);
	...
	for (i=0; i<num_ipoints-1; i+=3) {
		h2_bezier_draw_cubic(draw_segment, 15, pi1[i], pi1[i+1], pi1[i+2], pi1[i+3]);
	}
	gfx_set_clipping_area_max();
}
...
static inline void init_balls(void) {
    ball_setup(
    		&ball_simulation,
			mtime(),
			// the walls are changed in the draw_background function!
			(walls_t) {
    			.x1=201,
    			.y1=61,
				.x2=gfx_width()-4,
...
static inline void draw_balls(demo_mode_t demo_mode) {
	...
	if (ball_timeout < ctime) {
	    ...
	    ball_create(&ball_simulation,  6,30, (point2d_t){ 0,0 }, (point2d_t){ 100, 0 }, GFX_COLOR_RED);
	    // puts this ball outside the area..
	    ball_simulation.balls[ball_simulation.balls_count-1].pos = (point2d_t) {10,gfx_height()/2};
	}
	/* move balls */
...
static inline void draw_background(demo_mode_t demo_mode) {
    ...
    gfx_puts3(gfx_width()-10, 14, "Press the blue button to\nchange the demonstrations", GFX_ALIGNMENT_RIGHT);
