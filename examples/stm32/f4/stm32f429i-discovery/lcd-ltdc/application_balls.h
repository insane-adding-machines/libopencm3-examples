...
static inline
void
ball_setup(balls_t *self, uint64_t time_ms_0, walls_t walls, ball_t balls[], size_t balls_size) {
		...
		while (l) {
			if ((ball_hittest(&ball, b))) {
				...
				if (y_move < y_range) {
					...
					/* failed to fit ball into walls/balls! */
					hit = false;
				}
				break;
			}
...
ball_to_ball_elastic_collision(ball_t *b1, ball_t *b2) {
	...
	point2d_t v_n  = point2d_sub_ts(b2->pos, b1->pos); // v_n = normal vec. - a vector normal to the collision surface
	point2d_t v_un = point2d_unit(v_n); // unit normal vector
	...
	point2d_t v_v1tPrime = point2d_mul_t(v_ut, v1tPrime);
	point2d_t v_v2nPrime = point2d_mul_t(v_un, v2nPrime);
	point2d_t v_v2tPrime = point2d_mul_t(v_ut, v2tPrime);

	/* Set new velocities in x and y coordinates */
	b1->vel = (point2d_t) { v_v1nPrime.x + v_v1tPrime.x, v_v1nPrime.y + v_v1tPrime.y };
	b2->vel = (point2d_t) { v_v2nPrime.x + v_v2tPrime.x, v_v2nPrime.y + v_v2tPrime.y };
}
...
ball_find_earliest_collision(balls_t *self, ball_t **b1c, ball_t **b2c) {
	if (self->balls_count==0) 	return (collision_t) { .type=COLL__NONE };
	...
	while (1) {
		...
		if (c.type != COLL__NONE) {
			if ((earliest_collision.type == COLL__NONE) || (c.time_to < earliest_collision.time_to)) {
...
ball_move(balls_t *self, uint64_t time_ms) {
	...
	while (--max_collisions) {
		...
		if (c.time_to < dt) {
			 ...
			 */
			ball_advance_positions(self->balls, self->balls_count, c.time_to);
		...
		last_collision = c; // Update lastCollision
	}
...
ball_draw(balls_t *self) {
	...
	while (l--) {
		...
		buf[0]=48+b_id++%10;
		gfx_puts2(
				(int16_t)(b->pos.x) - font_Tamsyn5x9b_9.charwidth *gfx_get_font_scale()/2+1,
				(int16_t)(b->pos.y) - font_Tamsyn5x9b_9.lineheight*gfx_get_font_scale()/2,
				buf,
				&font_Tamsyn5x9b_9,
				GFX_COLOR_WHITE
			);
