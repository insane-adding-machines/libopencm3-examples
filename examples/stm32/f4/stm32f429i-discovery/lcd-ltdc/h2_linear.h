...
#define h2lin_float_t    float
#define h2lin_float_EPSILON   FLT_EPSILON
#define h2lin_float_MIN_VALUE 0.00000001
#define h2lin_float_abs       fabsf
#define h2lin_float_sqrt      sqrtf
#define h2lin_float_pow       powf
#define h2lin_float_atan2     atan2f
#define h2lin_float_cos       cosf
#define h2lin_float_min       fminf
#define h2lin_float_max       fmaxf
...
static inline
point2d_t
point2d_mul_t(point2d_t p, h2lin_float_t mul) {
	return (point2d_t) {mul * p.x, mul * p.y};
}
static inline
point2d_t
point2d_div_t(point2d_t p, h2lin_float_t div) {
...
point2d_norm(point2d_t v) {
	return h2lin_float_sqrt(point2d_dot(v, v));
}
static inline
h2lin_float_t
...
dist_point_to_line(point2d_t P, segment2d_t s) {
	...
	h2lin_float_t c1 = point2d_dot(w,v);
	h2lin_float_t c2 = point2d_dot(v,v);
	h2lin_float_t b  = c1 / c2;
...
dist_line_to_line(segment2d_t l1, segment2d_t l2) {
	...
	h2lin_float_t a = point2d_dot(u,u);        // always >= 0
	h2lin_float_t b = point2d_dot(u,v);
	h2lin_float_t c = point2d_dot(v,v);        // always >= 0
	h2lin_float_t d = point2d_dot(u,w);
	h2lin_float_t e = point2d_dot(v,w);
	h2lin_float_t D = a*c - b*b;       // always >= 0
	h2lin_float_t sc, tc;

	// compute the line parameters of the two closest points
	if (D < h2lin_float_MIN_VALUE) {         // the lines are almost parallel
		sc = 0.0;
	...
	h2lin_float_t ndP = point2d_norm(dP);
...
dist_segment_to_segment(segment2d_t s1, segment2d_t s2) {
	...
	h2lin_float_t e = point2d_dot(v,w);
	h2lin_float_t D = a*c - b*b;       // always >= 0
	h2lin_float_t sc, sN, sD = D;      // sc = sN / sD, default sD = D >= 0
	h2lin_float_t tc, tN, tD = D;      // tc = tN / tD, default tD = D >= 0
	...
	if (h2lin_float_abs(sN) < h2lin_float_MIN_VALUE) { sc = 0; } else { sc = sN / sD; }
	if (h2lin_float_abs(tN) < h2lin_float_MIN_VALUE) { tc = 0; } else { tc = tN / tD; }
	...
	/*  ???? */
	if (ndP < h2lin_float_MIN_VALUE) ndP = 0.0;
